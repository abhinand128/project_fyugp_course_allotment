...
class BaseCourseSelectionForm(forms.Form):
    ...
    def get_batch_year(self):
        """Calculate the academic year string based on student's admission year and current semester"""
        if not self.student:
            return None
        start_year = self.student.admission_year + (self.student.current_sem - 1) // 2
...
class CourseSelectionFormSem1(BaseCourseSelectionForm):
    def setup_fields(self):
        batch_year = self.get_batch_year()
        # Fetch Semester 1 batches with exact course type matching and year filtering
...
class CourseSelectionFormSem2(BaseCourseSelectionForm):
    def setup_fields(self):
        ...
        # Get previous semester allotments to handle pathway rules
        ...
        # Fetch Semester 2 batches with exact course type matching and year filtering
        dsc1_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC4',
            course__semester=2,
            year=batch_year,
            status=True
        )
        dsc2_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC5',
            course__semester=2,
            year=batch_year,
            status=True
        )
        dsc3_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC6',
            course__semester=2,
            year=batch_year,
            status=True
        )
        ...
        # DSC2 options - create exactly 3 options
        dsc2_count = min(dsc2_filtered.count(), 3)
        for i in range(1, dsc2_count + 1):
            self.create_batch_field(f'dsc_2_option_{i}', dsc2_filtered, f'DSC 2 Option {i}')

        # DSC3 options - create exactly 3 options
        dsc3_count = min(dsc3_filtered.count(), 3)
        for i in range(1, dsc3_count + 1):
    ...
    def get_field_order(self):
        ...
        # Add DSC2 options (up to 3)
        field_order += sorted(
            [f for f in self.fields if f.startswith('dsc_2_option_')],
            key=lambda x: int(x.split('_')[-1]))

        # Add DSC3 options (up to 3)
        field_order += sorted(
            [f for f in self.fields if f.startswith('dsc_3_option_')],
            key=lambda x: int(x.split('_')[-1]))
...
class CourseSelectionFormSem3(BaseCourseSelectionForm):
    def setup_fields(self):
        batch_year = self.get_batch_year()
        
        # 1. Majors: DSC7, DSC8 (Own Department only)
        # Options count = total available batches in student's department
        dsc7_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC7',
            course__semester=3,
            year=batch_year,
            status=True,
            course__department=self.student.department
        )
        dsc8_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC8',
            course__semester=3,
            year=batch_year,
            status=True,
            course__department=self.student.department
        ...
        # 2. Minors: DSC9, DSC10 (Not from student's department)
        # 3 options each
        dsc9_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC9',
            course__semester=3,
            year=batch_year,
            status=True
        ).exclude(course__department=self.student.department)
        
        dsc10_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='DSC10',
            course__semester=3,
            year=batch_year,
            status=True
        ).exclude(course__department=self.student.department)

        # 3. MDC and VAC (Any department)
        # Options count = total available batches
        mdc_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='MDC',
            course__semester=3,
            year=batch_year,
        ...
        vac_batches = Batch.objects.select_related('course', 'course__department').filter(
            course__course_type__name='VAC',
            course__semester=3,
            year=batch_year,
            status=True
        )

        # Create fields for DSC7 (Paper 1)
        for i in range(1, dsc7_batches.count() + 1):
            self.create_batch_field(f'dsc_1_option_{i}', dsc7_batches, f'DSC 7 Option {i}')
        
        # Create fields for DSC8 (Paper 2)
        for i in range(1, dsc8_batches.count() + 1):
            self.create_batch_field(f'dsc_2_option_{i}', dsc8_batches, f'DSC 8 Option {i}')

        # Create fields for DSC9 (Paper 3) - 3 options
        for i in range(1, min(dsc9_batches.count(), 3) + 1):
            self.create_batch_field(f'dsc_3_option_{i}', dsc9_batches, f'DSC 9 Option {i}')

        # Create fields for DSC10 (Paper 4) - 3 options
        for i in range(1, min(dsc10_batches.count(), 3) + 1):
            self.create_batch_field(f'dsc_4_option_{i}', dsc10_batches, f'DSC 10 Option {i}')

        # Create fields for MDC (Paper 5)
        for i in range(1, mdc_batches.count() + 1):
            self.create_batch_field(f'mdc_option_{i}', mdc_batches, f'MDC Option {i}')

        # Create fields for VAC (Paper 6)
        for i in range(1, vac_batches.count() + 1):
            self.create_batch_field(f'vac_option_{i}', vac_batches, f'VAC Option {i}')

    def get_field_order(self):
        field_order = []
        # Dynamic sorting based on field prefixes
        prefixes = ['dsc_1_option_', 'dsc_2_option_', 'dsc_3_option_', 'dsc_4_option_', 'mdc_option_', 'vac_option_']
        for prefix in prefixes:
            field_order += sorted(
                [f for f in self.fields if f.startswith(prefix)],
                key=lambda x: int(x.split('_')[-1])
            )
        return field_order
